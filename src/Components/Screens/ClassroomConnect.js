...
const GoogleClassroomIntegration = () => {
  ...
  return (
    <div className="google-classroom-div">
      ...
      <p>
        <i>
          To display the list of <b>upcoming assignments</b> and their{" "}
          <b>due date</b>, click on the <b>Connect</b> button <u></u>after
          logging in!
        </i>
      </p>
      <ul>
        {upcomingCourses.map((upcomingCourse, index) => (
          <li key={index}>
            <strong>{upcomingCourse.courseTitle}</strong>:{" "}
            {upcomingCourse.courseWorkTitle} - Due in{" "}
            {upcomingCourse.daysRemaining} days
          </li>
        ))}
      </ul>
    </div>
